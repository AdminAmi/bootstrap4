...
*/
    ...
    public void smjesti(ArrayList<Podaci> projekat){
        li.setProjekti(new ArrayList<Podaci>());
        for (Podaci o:projekat) li.getProjekti().add(o);
    }
    ...
    public  boolean smjestiUXML( ) {
        try {
            ...
            jaxbMarshaller.marshal(li, new File(utility.putZaXML + "mail.xml"));
            return true;
    ...
    public ArrayList<Podaci> procitajIzXMLa() throws JAXBException {
        JAXBContext jaxbContext = JAXBContext.newInstance(projekatOmotac.class);
        Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();     
        //Ucitava iz xml filea
        projekatOmotac emps = (projekatOmotac) 
            jaxbUnmarshaller.unmarshal( new File(utility.putZaXML + "mail.xml") );
        
        // Ispis iz xml-a na konzolu
        //for(Osoba os : emps.getListaOsoba()){System.out.println(os);}    
        return (ArrayList<Podaci>) emps.getProjekti();
    }
